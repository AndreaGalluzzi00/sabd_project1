...
Pipeline:
  ...
  3. Elbow + Silhouette per k in [2..6] → k ottimale = max silhouette
...
df = df_raw.filter(col("OP_UNIQUE_CARRIER").isin(carrier_list))
...
df_result = km_model.transform(df_scaled)
